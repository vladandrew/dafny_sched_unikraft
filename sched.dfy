...
predicate prefix_from(a : seq<Thread>, b : seq<Thread>, i : uint64)
{
    i <= |b| as uint64 && a == b[..i]
}
...
predicate suffix_from(a : seq<Thread>, b : seq<Thread>, i : uint64)
{
    i <= |b| as uint64 && a == b[i..]
}
...
function partition_point(a : seq<Thread>, wkt : uint64) : uint64
    reads a
    requires sorted(a)
    ensures 0 <= partition_point(a, wkt) <= |a| as uint64
    ensures forall i | 0 <= i < partition_point(a, wkt) :: a[i].wakeup_time <= wkt
    ensures forall i | partition_point(a, wkt) <= i < |a| as uint64 :: a[i].wakeup_time > wkt
{
...
class Thread {
    ...
    method set_wakeup_time(delta : uint64)
    modifies this
...
class ExitedQueue {
    ...
    function method get_at(i : uint64) : Thread
    reads this, q
    requires 0 <= i < Length()
    {
        q[i]
    ...
    method get_index(t : Thread) returns (i : uint64)
    ...
    method remove_at(i : uint64)
    modifies this
    requires Valid()
    requires 0 <= i < |q| as uint64
    ensures |q| == |old(q)| - 1
    ensures Valid()
    ensures this == old(this)
    {
...
class SleepingQueue {
    ...
    method bin_src(wkt : uint64) returns (ind : uint64)
        requires sorted(q)
        ensures ind >= 0 ==> ind < |q| as uint64 && q[ind].wakeup_time == wkt
        ensures ind < 0 ==> forall i | 0 <= i < |q| as uint64 :: q[i].wakeup_time != wkt
    {
        var l, r := 0, |q| as uint64;

        while l < r
            invariant 0 <= l <= r <= |q| as uint64
            invariant forall i | 0 <= i < l :: q[i].wakeup_time < wkt
            invariant forall i | r <= i < |q| as uint64 :: q[i].wakeup_time > wkt
        {
            var m := (l + r) / 2;
    ...
    method find_insert_point(wkt : uint64) returns (ind : uint64)
        requires Valid()
        ensures Valid()
        ensures 0 <= ind <= |q| as uint64
    ...
    method partition(wkt : uint64) returns (smaller : seq<Thread>)
        modifies this
        requires Valid()
        ensures Valid()
        ...
        ensures |smaller| as uint64 == partition_point(old(q), wkt)
...
class SchedCoop {
    ...
    method current_sleep(how_long : uint64) returns (next : Thread)
...
/*
method runnable_thread_will_run_complete(s : SchedCoop, target : Thread, should_sleep : seq<bool>, time_to_sleep : seq<uint64>, should_add_thread : seq<bool>)
